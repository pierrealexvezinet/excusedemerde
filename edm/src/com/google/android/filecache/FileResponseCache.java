...
public abstract class FileResponseCache extends ResponseCache {
    ...
    protected boolean isStale(File file, URI uri, String requestMethod,
    	...
    	//HACK: PCA: ugly hack: if it is an image, I'll always allow caching.
    	String path = uri.getPath();
    	if ( path != null) {
    		int dot = path.lastIndexOf('.');
    	    String ext = path.substring(dot + 1).toLowerCase();
    	    if ("jpeg".equals(ext) || "jpg".equals(ext) || "png".equals(ext) || "gif".equals(ext)) {
    	    	Log.d(TAG, "file "+path +" is an image, read from cache.");
    			return false;
    	    }
    	}
    ...
    private static class Frame {
        ...
        private static final int GINGERBREAD = 10;
